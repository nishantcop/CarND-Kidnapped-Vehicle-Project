...
// A random engine to be used across multiple and various method calls
...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  normal_distribution<double> dist_x(x, std[0]); // SD-X
  normal_distribution<double> dist_y(y, std[1]); // SD-Y
  normal_distribution<double> dist_theta(theta, std[2]); // SD-theta
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    for (unsigned int j = 0; j < trans_os.size(); j++) {
      ...
      particles[i].weight *= obs_w == 0 ? 0.00001 : obs_w;
    }
  }
}
